...
for the primitive variables at 1.1AU and propagates backwards to 30RS.
...
U_SOL_E[:, :, -1] = np.array((vr[:, 55, -1],  rho[:, 55, -1], Pr[:, 55, -1], vp[:, 55, -1]))


for ii in range(len(new_r)-1):
    U_SOL_E[:, :, -ii - 2] = backward_euler_pizzo_2d(U=U_SOL_E[:, :, -ii - 1],
                                                     dr=np.abs(new_r[-ii - 1] - new_r[-ii - 2]),
                                                     dp=dp, r=new_r[-ii - 1], theta=np.pi/2)
...
for j in sample_columns:
    ...
    #pressure
    ax[2].plot(180 / np.pi * p, (U_SOL_E[2, :, j] * (u.kg / ((u.s ** 2) * u.km))).to(u.dyne / (u.cm ** 2)), color=curr_color,
