...
for j in sample_columns:
    ...
    ax[0].plot(180 / np.pi * p, U_SOL_E[0, :, j], color=curr_color, linewidth=1.0,
               label="$v_{r}(\phi, r$ = " + str(round(((new_r[j]).to(u.AU)).value, 2)) + " AU)")
    ax[1].plot(180 / np.pi * p, ((U_SOL_E[1, :, j] / m_p.value) * (1 / u.km ** 3)).to(1 / u.cm ** 3), color=curr_color,
               linewidth=1.0, label="$n(\phi, r$ = " + str(round(((new_r[j]).to(u.AU)).value, 2)) + " AU)")
    ax[2].plot(180 / np.pi * p, (U_SOL_E[2, :, j] * (u.kg / ((u.s ** 2) * u.km))).to(u.dyne / (u.cm ** 2)), color=curr_color,
               linewidth=1.0, label="$p(\phi, r$ = " + str(round(((new_r[j]).to(u.AU)).value, 2)) + ")")
    ax[3].plot(180 / np.pi * p, U_SOL_E[3, :, j], color=curr_color, linewidth=1.0,
               label="$v_{\phi}(\phi, r$ = " + str(round(((new_r[j]).to(u.AU)).value, 2)) + ")")
